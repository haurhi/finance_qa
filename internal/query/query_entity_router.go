...
func extractNamedEntityFromQuestion(question string) string {
	...
	candidate := q
	replacements := []string{
		"帮我查一下", "帮我查", "查一下", "有哪些", "哪些", "哪个", "哪几个", "哪几家",
		"今年", "本年", "累计", "年内", "至今", "到现在", "当前", "目前", "最新", "最新可见月份", "可见月份", "最近完整月份", "最近月份", "其中", "到账", "回款", "收款", "付款", "支付", "收入", "营收", "销售额", "GMV", "gmv",
...
func looksLikeSyntheticQuestionFragment(entity string) bool {
	...
	questionFragments := []string{
		...
		"起至今", "至今", "到现在", "当前", "目前", "最新", "最新可见月份", "可见月份", "最近完整月份",
